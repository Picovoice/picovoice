...
export function getContextPathsByLanguage(language: string, context: string): string {
    return path.join(
        ROOT_DIR,
...
export function getKeywordPathsByLanguage(language: string, keyword: string): string {
    return path.join(
        ROOT_DIR,
...
export function getAudioFileByLanguage(language: string, audioFile: string): string {
    return path.join(
        ROOT_DIR,
...
export function getTestParameters(): [string, string, string, string, string, Record<string, string>][] {
  let parametersJson = TEST_DATA_JSON.tests.parameters;
  return parametersJson.map(
    (x: any) => [x.language, x.wakeword, x.context_name, x.audio_file, x.inference.intent, x.inference.slots]
  );
}
