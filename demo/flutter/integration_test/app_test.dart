...
void main() {
  ...
  group('Picovoice Tests', () {
    ...
    testWidgets('Test all languages',
      (tester) async {
        for (int t = 0; t < testData['tests']['parameters'].length; t++) {
          ...
          String keywordPath = "assets/test_resources/keyword_files/${wakeword}_${platform}.ppn";
          String contextPath = "assets/test_resources/context_files/${contextName}_${platform}.rhn";
          String porcupineModelPath = "assets/test_resources/model_files/porcupine_params${language != "en" ? "_${language}" : ""}.pv";
          String rhinoModelPath = "assets/test_resources/model_files/rhino_params${language != "en" ? "_${language}" : ""}.pv";
          ...
          try {
            ...
            expect(ex, equals(null), reason: "Failed to initialize Picovoice for ${language}: ${ex}");
            return;
          }
          ...
          picovoice.delete();
          expect(wakewordCalled, equals(true), reason: "Picovoice didn't call wakewordCallback for ${language} ${wakeword} ${contextName}");
          expect(inference, isNot(equals(null)), reason: "Picovoice returned wrong inference for ${language} ${wakeword} ${contextName}");
          expect(inference?.isUnderstood, equals(true), reason: "Picovoice returned wrong inference for ${language} ${wakeword} ${contextName}");
          expect(inference?.intent, equals(expectedInference['intent']), reason: "Picovoice returned wrong inference for ${language} ${wakeword} ${contextName}");
          expect(inference?.slots, equals(expectedInference['slots']), reason: "Picovoice returned wrong inference for ${language} ${wakeword} ${contextName}");
        }
    });
  });
}
