...
#include "pv_embedded/pv_st_h747.h"
...
int main(void) {
    ...
    check_status(status);
    pv_message_send(PV_MESSAGE_CODE_HANDSHAKE, PV_BOARD_NAME);
    ...
    static const int32_t REPETITION = 1;
    ...
    pv_message_send(PV_MESSAGE_CODE_INFO, "Non-idle-case: processed %lu milliseconds of audio in %lu milliseconds (%ld Percent)",
            test_msec,
            (int32_t) pv_profile_get_elapsed_msec(profile),
            (int32_t) (100 * pv_profile_get_elapsed_msec(profile)) / test_msec);
