...
def update_ios_demo(parameters):

    scheme_dir = os.path.join(
        os.path.dirname(__file__),
        "../../demo/ios/ForegroundApp/PicovoiceForegroundAppDemo.xcodeproj/xcshareddata/xcschemes")
    ...
    for language, parameter in parameters.items():
        ...
        if not os.path.exists(language_scheme):
            ...
            for env_var in env_vars:
                ...
                if env_var.getAttribute('key') == 'WAKEWORD':
                    env_var.setAttribute('value', parameter['wakeword'])
                if env_var.getAttribute('key') == 'CONTEXT':
                    env_var.setAttribute('value', parameter['context'])
...
def main():
    ...
    parameters = {x['language']: {'wakeword': x['wakeword'], 'context': x['context_name']} for x in json_content['tests']['parameters']}
    update_ios_demo(parameters=parameters)
