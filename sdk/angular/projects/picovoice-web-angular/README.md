...
**Important Note** The @picovoice/picovoice-web-${LANGUAGE}-\* series of packages are on the order of ~4-6MB, as they contain the entire Voice AI model. Typically, you do _not_ want to import these statically, as your application bundle will be much larger than recommended. Instead, use dynamic imports so that the chunk is lazy-loaded:

```typescript
  async ngOnInit() {
    // Load Picovoice worker chunk with specific language model (large ~1-2MB chunk; dynamically imported)
...
Upon mounting, the component will request microphone permission from the user, instantiate the audio stream, start up an instance of Picovoice, and listen for both "Picovoice" and "Okay Google". When the words are detected, they will be logged to the console.
...
Custom wake words are generated using [Picovoice Console](https://picovoice.ai/console/). They are trained from text using transfer learning into bespoke Picovoice keyword files with a `.ppn` extension. The target platform is WebAssembly (WASM), as that is what backs the Angular library.

Convert the `.ppn` file to base64 and provide it as an argument to Picovoice as below. You will need to also provide a label so that the PicovoiceService can tell you which keyword occurred ("Deep Sky Blue", in this case):
...
```javascript
  await this.picovoiceService.init(pvFactoryEn,
    {
      // Custom Porcupine keyword (Base64 representation of a `.ppn` file)
      porcupineKeyword: { custom: "Deep Sky Blue", base64: DEEP_SKY_BLUE_PPN_64 },
      // Rhino context (Base64 representation of a `.rhn` file)
      rhinoContext: { base64: RHINO_CLOCK_64 },
      start: true
    })
